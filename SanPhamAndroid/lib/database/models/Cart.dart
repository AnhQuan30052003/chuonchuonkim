...
class CartSnapshot {
  ...
  static Stream<List<CartSnapshot>> streamData() {
    var querySnapshot = FirebaseFirestore.instance.collection(Firebase.colCart).where("idUser", isEqualTo: ChuonChuonKimController.instance.user!.id).snapshots();
  ...
  static Future<List<CartSnapshot>> futureData() async {
    var qs = await FirebaseFirestore.instance.collection(Firebase.colCart).where("idUser", isEqualTo: ChuonChuonKimController.instance.user!.id).get();
